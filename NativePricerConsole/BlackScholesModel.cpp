...
BlackScholesModel::BlackScholesModel(int size, double r, PnlMat *CovLogR,PnlVect *spot,PnlVect * trend ) {
	...
	sigma_ = vol;
	double corij;
	PnlMat* cor = pnl_mat_create_from_scalar(size, size, 1.0);
	for (int i = 0; i < size; i++) {
		for (int j = 0; j < i; j++) {
			corij = pnl_mat_get(CovLogR, i, j) / (pnl_vect_get(vol, i)* pnl_vect_get(vol, j));
...
void BlackScholesModel::set(size_t size, double r, double rho, PnlVect * sigma, PnlVect * spot, PnlVect * trend)
{
	size_ = size;
	r_ = r;
	rho_ = rho;
